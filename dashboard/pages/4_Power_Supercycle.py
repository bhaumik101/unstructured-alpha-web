...
from utils.config import SIGNALS, POWER_SUPERCYCLE_SIGNALS, POWER_SUPERCYCLE_TICKERS, CATEGORIES
...
# ── Multi-Leg Signal Chart — small multiples, one panel per leg ───────────────
st.markdown("### Signal Trends by Leg")
st.caption("Each series normalized to 100 at the start of the selected window. Split into one panel per leg so individual trends stay readable.")
...
with st.expander("Reading these charts"):
    st.markdown(f"""
    Cramming all eleven signals onto a single chart makes individual trends impossible to read,
    so each leg of the thesis gets its own panel below — each series is normalized to 100 at the
    **start of the selected window** ({leg_period}), grouped so the lines that belong together are
    easy to compare.

    **What to look for:**
    - Lines **rising together within a panel** = that leg of the thesis is strengthening
    - Lines **diverging within a panel** = that leg is mixed, not a clean signal yet
    - **Most panels trending the same direction at the same time** = the strongest form
      of confluence — the whole supercycle thesis moving together, not just one commodity

    **A note on the short windows (1H / 1D / 1W):** most of these signals are macro data
    series published daily, weekly, monthly, or even quarterly (e.g. hyperscaler capex) —
    not intraday ticks. Selecting a very short window will show only the handful of data
    points that exist within it, which is an honest reflection of how often that data is
    actually published, not a rendering error.

    The **confluence of multiple independent signals** is what makes this analysis
    different from just watching uranium or copper in isolation.
    """)

leg_colors = {
    "uranium_proxy":     "#4A1B6B",
    "copper":            "#B34700",
    "natural_gas":       "#B8860B",
    "hyperscaler_capex": "#1C2B4A",
    "semiconductor_etf": "#0D4F5C",
    "ata_trucking":      "#1B5E20",
    "jobless_claims":    "#7B1010",
    "ism_pmi":           "#5D4037",
    "crude_oil":         "#8B7355",
}
...
def _window_and_resample(s: pd.Series, period: str) -> pd.Series:
    """Trim a raw signal series to the selected window, then resample to a
    sensible granularity for that window length. Falls back to the most
    recent few raw points if the window is too short for the signal's
    native publication frequency (e.g. a monthly series under a 1D window)."""
    ...
    if len(windowed) < 2:
        # Not enough native data points in this window — fall back to the
        # last few available readings so the panel still shows a line.
        windowed = s.tail(min(6, len(s)))

    # Resample for smoothing on longer windows; keep native resolution on short ones.
    resample_rule = "D" if period in ("1H", "1D", "1W") else ("W" if period in ("1M", "3M") else "W")
    try:
...
_LEG_COLS = 3
_n_legs   = len(leg_map)
_n_rows   = -(-_n_legs // _LEG_COLS)  # ceil division
...
# ── Ticker Performance by Supercycle Leg ─────────────────────────────────────
st.markdown("### Ticker Performance by Supercycle Leg")
st.caption("How each basket of supercycle-relevant tickers has performed over the selected period.")

with st.expander("Why these specific tickers?"):
    st.markdown("""
    Each leg of the Power Supercycle thesis has specific publicly traded companies that
    *benefit most* if that leg of the thesis plays out:

    | Leg | Key Tickers | Why |
    |---|---|---|
    | Nuclear Fuel Chain | CCJ, LEU, UEC, URA | Uranium miners and enrichers benefit from rising fuel prices |
    ...
    | Gas Pipelines | WMB, KMI, OKE | Data center co-located gas demand = long-term contracted revenue |
    | AI / Hyperscalers | NVDA, MSFT, AMZN | Direct beneficiaries of AI compute buildout |

    The thesis: if you believe the Power Supercycle is playing out, you want exposure across
    multiple legs — not just one. Owning only uranium is a one-leg bet.
    """)
...
# ── Clickable ticker chips per basket ─────────────────────────────────────────
...
# ── Uranium CFTC COT ──────────────────────────────────────────────────────────
st.markdown("### Copper COT — Commercial Positioning")

with st.expander("Why copper COT matters for the supercycle"):
    st.markdown("""
    Copper is the most critical physical material in the Power Supercycle thesis.

    **Commercial hedgers in copper COT** are the copper miners, manufacturers, and utilities
    that actually use or produce copper. When they take **extreme net long positions**, they're
    locking in purchase prices because they expect copper to be scarce — and expensive.

    This is one of the clearest "smart money" signals available in commodity markets.
    When commercial positioning hits the 80th percentile or above, it has historically
    preceded significant copper price appreciation.

    **The AI/data center angle:** Copper demand from grid buildout overtook EVs as the primary
    growth driver in June 2026. Commercial hedgers in copper see this demand in their order books
    *months before* it shows up in macro data.
    """)
...
cot_color  = STATUS_COLOR.get(cot_status, "#FF9800")
...
if not copper_cot.empty:
    ...
    fig_cot.update_layout(
        ...
        height=280, barmode="group", paper_bgcolor="#FAF7F0", plot_bgcolor="#FFFFFF",
...
# ── arXiv Quantum Paper Velocity ──────────────────────────────────────────────
st.markdown("### Quantum Computing — arXiv Paper Velocity")
...
with st.expander("Why arXiv paper velocity is a leading indicator"):
    ...
    **The signal:**
    ...
    - When Google published the Willow chip paper (below-threshold error correction),
      institutional money rotated into quantum stocks **within weeks** of the preprint
    - A sustained spike in paper publication velocity from major labs (Google Quantum AI,
      IBM Research, Microsoft Station Q) signals an imminent milestone announcement

    **What "below-threshold error correction" means for investors:**
    This is the critical technical hurdle for fault-tolerant quantum computing. When any major
    lab achieves this reliably, it unlocks commercial viability — and a major re-rating of
    pure-play quantum stocks (IONQ, RGTI, QBTS).
...
if not arxiv_series.empty:
    ...
    fig_arx.update_layout(
        ...
        height=280, paper_bgcolor="#FAF7F0", plot_bgcolor="#FFFFFF",
        xaxis=dict(showgrid=True, gridcolor="#E8E0CE", tickfont=dict(color="#6B6560")),
        yaxis=dict(showgrid=True, gridcolor="#E8E0CE", tickfont=dict(color="#6B6560"),
...
# ── Nuclear Federal Contracts ─────────────────────────────────────────────────
st.markdown("### DoE Nuclear Contract Awards")
...
with st.expander("Why DoE contract awards are a leading indicator for nuclear stocks"):
    ...
    **Why it matters for investors:**
    ...
    Contract award velocity for companies like Centrus Energy (LEU) and Cameco (CCJ) is one
    of the most reliable leading indicators for nuclear equity performance.
    """)
...
nuke_cols = st.columns(len(nuclear_companies))
for col, (display_name, search_name) in zip(nuke_cols, nuclear_companies.items()):
    with col:
        with st.spinner(f"Loading {display_name}…"):
            contracts = fetch_federal_contracts(search_name, years=2)
            vel       = score_contract_velocity(contracts)
        ...
        <div style="background:#F0EBE1;border-radius:6px;padding:14px;
            ...
            <div style="font-size:1.4rem;font-weight:700;color:{vel_color};">
                {vel_pct:+.1f}%
            </div>
            <div style="font-size:0.75rem;color:#6B6560;">
                Recent 6m: ${vel_recent:,.0f}<br>
                Prior 6m: ${vel_prior:,.0f}
            </div>
        </div>
        """, unsafe_allow_html=True)
...
st.markdown("### Full Signal Confluence Summary")
