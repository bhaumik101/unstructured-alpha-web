...
STUB_CATEGORIES = {"macro": {"name": "Macro & Liquidity"}, "financials": {"name": "Financials & Credit"}}
STUB_SIGNALS = {
...
def _stub_model_config(monkeypatch):
    monkeypatch.setattr(mv, "SIGNALS", STUB_SIGNALS)
    monkeypatch.setattr(mv, "CATEGORIES", STUB_CATEGORIES)


def test_core_tier1_record():
    r = mv.signal_validation_record("ata_trucking", STUB_SIGNALS["ata_trucking"])
...
def test_experimental_tier3_record():
    r = mv.signal_validation_record("retail_fear_gauge", STUB_SIGNALS["retail_fear_gauge"])
...
def test_measured_reliability_overrides():
    hi = mv.signal_validation_record("ata_trucking", STUB_SIGNALS["ata_trucking"],
                                     reliability={"score": 75, "label": "Reliable"})
    assert hi["confidence"] == "High"
    assert hi["validation_status"] == "Reliable"
    lo = mv.signal_validation_record("ata_trucking", STUB_SIGNALS["ata_trucking"], reliability={"score": 30})
    assert lo["confidence"] == "Limited"


def test_table_sorting_and_wrapper():
    recs = mv.build_validation_table(STUB_SIGNALS)
    ...
    recs2 = mv.build_validation_table(STUB_SIGNALS, reliabilities={"ata_trucking": {"reliability": {"score": 80}}})
