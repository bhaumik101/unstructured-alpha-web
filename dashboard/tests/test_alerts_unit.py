...
not re-testing the scoring pipeline itself.
"""
...
from utils import alerts_db, alerts
...
def test_evaluate_ticker_first_check_creates_no_alerts_but_saves_state():
    ...
    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(50.0, 100.0)):
        result = alerts.evaluate_ticker("CCJ", _THRESHOLDS)
    assert result == []
    state = alerts_db.get_alert_state("CCJ")
...
def test_evaluate_ticker_score_crossing_into_bullish_fires_once():
    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(60.0, 100.0)):
        alerts.evaluate_ticker("CCJ", _THRESHOLDS)  # baseline: 60, below 65 threshold

    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(70.0, 100.0)):
        result = alerts.evaluate_ticker("CCJ", _THRESHOLDS)  # crosses above 65
...
def test_evaluate_ticker_staying_bullish_does_not_re_alert():
    ...
    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(70.0, 100.0)):
        alerts.evaluate_ticker("CCJ", _THRESHOLDS)  # baseline already bullish

    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(72.0, 100.0)):
        result = alerts.evaluate_ticker("CCJ", _THRESHOLDS)  # still bullish, didn't cross
...
def test_evaluate_ticker_score_crossing_into_bearish():
    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(40.0, 100.0)):
        alerts.evaluate_ticker("CCJ", _THRESHOLDS)
...
def test_evaluate_watchlist_evaluates_every_watched_ticker():
    alerts_db.add_to_watchlist("CCJ")
    alerts_db.add_to_watchlist("MSFT")
    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(50.0, 100.0)):
        result = alerts.evaluate_watchlist()
    assert result == []  # first check for both, no alerts, but...
    assert alerts_db.get_alert_state("CCJ") is not None
    assert alerts_db.get_alert_state("MSFT") is not None


def test_evaluate_watchlist_skips_tickers_that_error():
    alerts_db.add_to_watchlist("GOOD")
    alerts_db.add_to_watchlist("BAD")
    ...
    with patch("utils.alerts.compute_full_ticker_score", return_value=_fake_full(60.0, 100.0)):
        alerts.evaluate_ticker("GOOD", _THRESHOLDS)  # baseline
    ...
    assert alerts_db.get_alert_state("BAD") is None  # never successfully evaluated
